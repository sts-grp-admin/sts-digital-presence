...
const ClientLogoCard = ({ name, logo, showName = false }: ClientLogoCardProps) => (
  <div className="bg-card border border-border rounded-lg p-6 flex flex-col items-center justify-center h-28 hover:border-primary/40 hover:shadow-sm transition-all duration-300 group">
    <img
      src={logo}
      alt={name}
      className="max-h-[44px] w-auto object-contain grayscale group-hover:grayscale-0 transition-all duration-300"
    />
    {showName && (
